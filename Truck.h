 ...
 **
 ** Constructs the truck and manages the trucks functions that allows
 ** the program to manipulate the queue and carry out each delivery
 ** 
 **
 **************************************/
#ifndef TRUCK_H
...
template <class T, int N>
  ...
  m_capacity = capacity;
  m_time = 0;
  m_curDelivery.resize(0);
}

// ~Truck ()
// Truck Destructor
template <class T, int N> //do something
  Truck<T, N>::~Truck() {
}
...
// AddDelivery ()
// Adds delivery to delivery vector
template <class T, int N>
...
// CompleteDelivery ()
// Completes a delivery (looped)
template <class T, int N>
  void Truck<T, N>::CompleteDelivery() {
  Item obj1;
  m_item.queueFront(obj1);
  cout << obj1.GetName();
  m_item.dequeue(obj1);
}
...
// GetDelivery ()
// Returns vector of current deliveries
template <class T, int N>
...
// SetTime ()
// Sets amount of time truck has been deliverying to customers
template <class T, int N>
